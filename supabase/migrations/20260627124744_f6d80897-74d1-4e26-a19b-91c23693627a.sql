...
-- 1) Adiciona label 'servico' nas funções de notificação e evita duplicidade
CREATE OR REPLACE FUNCTION public.enqueue_driver_status_notification(_equipment_id uuid, _new_status text, _previous_status text DEFAULT NULL::text, _driver_name text DEFAULT NULL::text, _extra_info text DEFAULT NULL::text, _water_point text DEFAULT NULL::text)
 RETURNS void
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  _new_label := CASE _normalized_new
    ...
    WHEN 'almoco' THEN '🍽️ Almoço'
    WHEN 'servico' THEN '🛠️ Serviço'
  ...
  WHERE origin = 'driver-status'
    ...
    AND (_water_point IS NULL OR message LIKE '%' || trim(_water_point) || '%')
    AND (_extra_info IS NULL OR message LIKE '%' || trim(_extra_info) || '%');
...
-- 2) Trigger de mudança de status: ignorar 'servico' (notificado pelo app com nome do serviço)
...
BEGIN
  ...
  -- 'servico' é enviado pelo app com o nome do serviço selecionado; não notifica genérico
  IF _new_status = 'servico' THEN
    RETURN NEW;
  END IF;
