...
BEGIN
  ...
  -- DEDUP REFORÇADO: se a última notificação enviada/enfileirada nas últimas 6h
  -- para este equipamento já era do mesmo status, ignora cliques repetidos.
  SELECT message INTO _last_status_label
  ...
  IF _last_status_label IS NOT NULL
     AND _last_status_label LIKE '%' || _new_label || '%'
     AND (_water_point IS NULL OR _last_status_label LIKE '%' || trim(_water_point) || '%') THEN
    RETURN;
  END IF;
