-- Update rpc_claim_equipment to return more info on failure
...
BEGIN
    ...
    IF v_equipment.driver IS NOT NULL AND v_equipment.driver != '' AND v_equipment.driver != p_driver_name THEN
        RETURN jsonb_build_object(
            'ok', false, 
            'error', 'Este equipamento já foi selecionado por ' || v_equipment.driver || '.',
            'driver_name', v_equipment.driver,
            'equipment_name', v_equipment.name,
            'shift_start_time', v_equipment.updated_at
        );
    END IF;
    ...
    IF FOUND THEN
        RETURN jsonb_build_object(
            'ok', false, 
            'error', 'Este equipamento possui um turno ativo iniciado por ' || v_active_shift.driver_name || '.',
            'driver_name', v_active_shift.driver_name,
            'equipment_name', v_active_shift.equipment_name,
