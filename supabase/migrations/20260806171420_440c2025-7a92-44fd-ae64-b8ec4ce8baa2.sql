-- 1) Limpa a fila atual (flood do reset administrativo)
DELETE FROM public.wapi_outbox
WHERE status IN ('pending', 'processing')
  AND origin IN ('daily-shift-report', 'driver-status');

-- 2) Trigger: só envia quando o motorista realmente encerra o turno
...
BEGIN
  ...
  -- Somente o Fim de Turno feito pelo motorista (que preenche os dados finais)
  IF NEW.final_horimeter IS NULL AND NEW.final_km IS NULL
     AND NULLIF(trim(COALESCE(NEW.final_fuel_level, '')), '') IS NULL THEN
    RETURN NEW;
  END IF;

  IF NULLIF(trim(COALESCE(NEW.driver_name, '')), '') IS NULL THEN
    RETURN NEW;
  END IF;
  ...
  -- Nunca reenviar a Parte Diária do mesmo turno
  IF EXISTS (
