...
BEGIN
  ...
  -- Janela de "reset diário" às 06:00h Pará (UTC-3) => 09:00 UTC
  _reset_cutoff := date_trunc('day', now() - interval '3 hours') + interval '9 hours';
  IF now() < _reset_cutoff THEN
    _reset_cutoff := _reset_cutoff - interval '1 day';
  END IF;
  ...
  -- Aproveita pending APÓS o reset (não reaproveita pending antigo de turnos anteriores)
  ...
  -- Bloqueia somente se já existir envio APÓS o último reset (06:00h Pará)
  IF EXISTS (
