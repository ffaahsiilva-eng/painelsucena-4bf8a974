// Devolve link de download direto do ZIP no Drive (alt=media) usando token da service account.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!job.drive_file_id) throw new Error("Backup sem arquivo no Drive");

    const r = LOVABLE_API_KEY && GOOGLE_DRIVE_API_KEY
      ? await fetch(
        `${DRIVE_GATEWAY_BASE}/drive/v3/files/${job.drive_file_id}?alt=media&supportsAllDrives=true`,
        {
          headers: {
    ...
    await admin.from("backup_audit_log").insert({
      ...
      details: { drive_path: job.drive_path },
    });
    ...
    return new Response(r.body, {
      headers: {
        ...
        "Content-Disposition": `attachment; filename="${
          job.drive_path?.split("/").pop() || "backup.zip"
        }"`,
      },
    });
  } catch (e) {
